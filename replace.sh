...
's/\@subsection Embedded WebKit Widgets$/\@subsection 埋め込みWebKitウィジェット/;' -i misc-ja.texi
fi
if [ $1 = "package-ja.texi" ]; then
    perl -pe 's/\@section Package Installation$/\@section パッケージのインストール/' -i package-ja.texi
fi
if [ $1 = "custom-ja.texi" ]; then
    perl -pe \
...
if [ $1 = "docstyle.texi" ]; then
    perl -0pe 's/UTF-8\n\@c/UTF-8\n\@documentlanguage ja\n\@c/m' -i docstyle.texi
fi
if [ $1 = "emacs-ja.texi" ]; then
